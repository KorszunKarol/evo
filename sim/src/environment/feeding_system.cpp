        ...
        if (diet.type == DietType::Herbivore) {
            ...
            spatial_index->for_each_in_radius(
                ...
                [&](entt::entity plant_entity, double distance_sq) {
                    ...
                    // Telemetry: track energy gained and successful feeds
             ...
             for (auto prey_entity : prey_view) {
                 ...
                 if (dist_sq <= reach_sq) {
                     ...
                     if (transferable > 0.0) {
                         ...
                         // Telemetry: track kills and energy gained
                         if (auto* telem = registry.try_get<TelemetryComponent>(entity)) {
                             telem->total_energy_gained += transferable;
                             ++telem->successful_feeds;
                             if (prey_metabolism.energy <= 0.0) {
                                 ++telem->kill_count;
                             }
                         }
                         
                         // Mark prey for predation death if energy depleted
                         if (prey_metabolism.energy <= 0.0) {
                             if (auto* prey_telem = registry.try_get<TelemetryComponent>(prey_entity)) {
                                 prey_telem->killed_by_predation = true;
                                 prey_telem->death_cause = DeathCause::Predation;
                             }
                         }
